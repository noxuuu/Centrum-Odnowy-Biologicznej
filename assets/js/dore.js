require('../css/dore.light.blue.css');
const $ = require('jquery');
const {copyImageLink, deleteImage, deleteAlbum, deletePortfolioAlbum} = require("./ajax");
/* Dore Main Script
...
$.dore = function(element, options) {
  ...
  function init() {
    ...
    if ($().contextMenu) {
      $.contextMenu({
        selector: ".list .context-item-album",
        callback: function(key, options) {
          switch (key) {
            case 'delete': {
              deleteAlbum(options.$trigger.attr('id'));
              break;
            }
          }
        },
        ...
        items: {
          delete: { name: "Usuń", className: "simple-icon-trash" }
        }
      });
      $.contextMenu({
        selector: ".list .context-item-photo",
        callback: function(key, options) {
          switch (key) {
            case 'copy_link': {
              copyImageLink(options.$trigger.attr('id'));
              break;
            }
            case 'delete': {
              deleteImage(options.$trigger.attr('id'));
              break;
            }
          }
        },
        ...
        items: {
          copy_link: { name: "Kopiuj link", className: "simple-icon-docs" },
          delete: { name: "Usuń", className: "simple-icon-trash" }
        }
      });
      $.contextMenu({
        selector: ".list .context-item-portfolio-album",
        callback: function(key, options) {
          switch (key) {
            case 'delete': {
              deletePortfolioAlbum(options.$trigger.attr('id'));
              break;
            }
          }
        },
        ...
        items: {
          delete: { name: "Usuń z portfolio", className: "simple-icon-trash" }
        }
      });
    }
